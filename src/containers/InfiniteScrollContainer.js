...
class InfiniteScrollContainer extends React.Component {
  ...
  getElements = () => {
    ...
    const campaignId =
      this.props.getACampaignDetailsResponse &&
      this.props.getACampaignDetailsResponse.campaignDetails.campaignDetails._id;
              ...
              <ListItemText
                ...
                primary={
                  <Tooltip title={value.locationNm}>
                    <Typography style={{ float: 'left', width: '80px', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                      {value.locationNm}
                    </Typography>
                  </Tooltip>
                }
  ...
  loadMore = () => {
    const campaignId =
      ...
      this.props.getACampaignDetailsResponse.campaignDetails.campaignDetails._id;
    const campaignEntries = this.props.getACampaignDetailsResponse && this.props.getACampaignDetailsResponse.allEntries;
    const lastCreatedAtOfPreviousPage =
      campaignEntries.length > 0 ? campaignEntries[campaignEntries.length - 1].createdAt : '';
    this.props.getCampaignDetailsById(campaignId, lastCreatedAtOfPreviousPage);
  };

  render() {
    return (
      <Paper style={paper} id="scrollableDiv">
        <List>
