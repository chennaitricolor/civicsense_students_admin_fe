...
export default function* getAcceptedEntriesForReportSaga(action) {
  ...
  try {
    ...
    campaignId =
      action.payload.campaignId !== '' && action.payload.campaignId !== undefined ? action.payload.campaignId : '';
    applyLimitValue =
      action.payload.applyLimit !== '' && action.payload.applyLimit !== undefined ? action.payload.applyLimit : true;
