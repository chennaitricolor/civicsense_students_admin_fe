...
export const MapWrappedComponent = withScriptjs(withGoogleMap(props => {
    ...
    const getAcceptedEntriesByDates = (acceptedEntries) => {
        if(props.selectedDate !== null) {
            return acceptedEntries.reportDetails.filter(acceptedEntry => isSameDates(formatDateToMMDDYYYYFormat(acceptedEntry.createdAt), props.selectedDate));
        }
